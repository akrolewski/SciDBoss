...
t0 = time.time()
qso_list = 'boss_test_sample.fits'
...
# we probably should have a redshift cut at (1216/1185)*zmax.  but it doesn't really matter for now
# maybe even at (1216/1041)*zmax.
...
#mean_spec = np.transpose([np.arange(min_wv,max_wv,wv_bin),np.zeros((max_wv-min_wv)/wv_bin)])
#mean_spec = mean_spectrum(all_qso, mean_spec)

#fz = np.transpose([np.arange(zmin,zmax,deltaz), np.zeros((zmax-zmin)/deltaz), np.zeros((zmax-zmin)/deltaz)])
#output = find_fz(all_qso, mean_spec, fz, forest_low_limit, forest_high_limit, lya)

#continuum = np.transpose([np.arange(min_wv,max_wv,wv_bin),np.ones((max_wv-min_wv)/wv_bin)])
...
for i in range(len(all_qso.values())):
	#if i == 2000 or i == 3000:
	#if i == 3000:
	spectrum = all_qso.values()[i][0]
	z = all_qso.values()[i][1]
	beta = a_qso[i]
	...
	plt.savefig(dir+'%i-%i-%i.pdf' % (all_qso.keys()[i][0],all_qso.keys()[i][1],all_qso.keys()[i][2]))
	#plt.xlim([1000,2200])
	#plt.ylim([-2,6])
