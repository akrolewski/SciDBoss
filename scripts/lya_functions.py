...
def fnorm(all_qso, norm_range_low, norm_range_high, lya):
	...
	for i in range(len(all_qso.values())):
		...
		#all_qso.keys()[i]
		#if all_qso.keys()[i] == (6530,332,56537):
		#	logl = spectrum[:,0][(rest_wavelength > norm_range_low) & (rest_wavelength < norm_range_high)]
		#	gflux = flux[(rest_wavelength > norm_range_low) & (rest_wavelength < norm_range_high)]
		#	ivar = spectrum[:,2][(rest_wavelength > norm_range_low) & (rest_wavelength < norm_range_high)]
		#	gweight = weight[(rest_wavelength > norm_range_low) & (rest_wavelength < norm_range_high)]
		#	for j in range(len(logl)):
		#		print logl[j],gflux[j],ivar[j],gweight[j]
